#!/bin/bash
#SBATCH --job-name=window-fine-2p5x
#SBATCH -p gpu
#SBATCH -N 1
#SBATCH --gres=gpu:1
...
#SBATCH --array=0-12%4
#SBATCH --output=logs/window_fine_%A_%a.out
#SBATCH --error=logs/window_fine_%A_%a.err

set -euo pipefail

WINDOW_SIZES=(96 128 160 192 224 256 288 320 352 384 400 416 448)
OVERLAP_RATIOS=(0.10 0.15 0.20 0.25 0.30 0.35 0.40 0.45 0.50 0.55 0.60 0.65 0.70)
...
for ((CELL_ID=TASK_ID; CELL_ID<169; CELL_ID+=13)); do
    SIZE_INDEX=$((CELL_ID / 13))
    OVERLAP_INDEX=$((CELL_ID % 13))
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/outputs/dino_window_supplement/05_window_sensitivity_2p5x_fine}"
