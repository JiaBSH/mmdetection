# Mask2Former for dense overlapping instance segmentation.
#
# Mask2Former is a transformer-based universal segmentation model.
# Its mask attention and bipartite matching naturally handle overlapping
# instances — each query attends to a specific instance regardless of
# spatial overlap with others.
#
# Overlap changes vs configs/custom/:
#   - test_cfg: lower score_thr (0.3→0.1), increase max_per_img (100→300)
#   - The model itself needs minimal changes since transformer queries
#     already avoid the NMS problem
...
batch_augments = [
    dict(type='BatchFixedSizePad', size=image_size,
         img_pad_value=0, pad_mask=True, mask_pad_value=0, pad_seg=False)
]
...
model = dict(
    ...
    panoptic_fusion_head=dict(
        num_things_classes=num_things_classes,
        num_stuff_classes=num_stuff_classes),
    # ── Overlap: lower score threshold, more instances ──
    test_cfg=dict(
        panoptic_on=False,
        semantic_on=False,
        instance_on=True,
        max_per_image=100,       # 300 → 100: model has 100 queries, k cannot exceed num_queries * num_things_classes
        object_mask_thr=0.5,     # keep default mask threshold
        iou_thr=0.8,
...
default_hooks = dict(
    checkpoint=dict(type='CheckpointHook', interval=1, by_epoch=True,
                    max_keep_ckpts=1, save_optimizer=False))
train_cfg = dict(
