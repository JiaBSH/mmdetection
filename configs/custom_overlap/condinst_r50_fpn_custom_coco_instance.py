# CondInst for dense overlapping instance segmentation.
#
# CondInst uses dynamic convolution (similar to SOLOv2) conditioned on
# FCOS detection. Its mask head generates per-instance convolution kernels,
# naturally avoiding the need for ROI-level NMS on masks.
#
# Overlap changes vs configs/custom/:
#   - test_cfg: score_thr=0.001 (from 0.05), max_per_img=300 (from 100)
#   - nms with higher iou_threshold for bbox stage
_base_ = '../../configs/condinst/condinst_r50_fpn_ms-poly-90k_coco_instance.py'

load_from = ('https://download.openmmlab.com/mmdetection/v3.0/condinst/'
...
model = dict(
    ...
    test_cfg=dict(
        nms=dict(type='nms', iou_threshold=0.7),  # 0.6 → 0.7: softer NMS
