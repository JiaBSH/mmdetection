# QueryInst for dense overlapping instance segmentation.
#
# QueryInst uses dynamic convolution heads across 6 stages with
# sparse learnable proposals. The iterative refinement over 6 stages
# helps resolve overlapping instances progressively.
#
...
_base_ = '../../configs/queryinst/queryinst_r50_fpn_1x_coco.py'
...
model = dict(
    roi_head=dict(
        bbox_head=[
            dict(type='DIIHead', num_classes=num_classes, num_ffn_fcs=2,
                num_heads=8, num_cls_fcs=1, num_reg_fcs=3,
                feedforward_channels=2048, in_channels=256, dropout=0.0,
                ffn_act_cfg=dict(type='ReLU', inplace=True),
                dynamic_conv_cfg=dict(type='DynamicConv', in_channels=256,
                    feat_channels=64, out_channels=256, input_feat_shape=7,
                ...
                bbox_coder=dict(type='DeltaXYWHBBoxCoder', clip_border=False,
        ...
        mask_head=[
            dict(type='DynamicMaskHead',
                dynamic_conv_cfg=dict(type='DynamicConv', in_channels=256,
                    feat_channels=64, out_channels=256, input_feat_shape=14,
                    with_proj=False, act_cfg=dict(type='ReLU', inplace=True),
                    norm_cfg=dict(type='LN')),
                num_convs=4, num_classes=num_classes, roi_feat_size=14,
                in_channels=256, conv_kernel_size=3, conv_out_channels=256,
                class_agnostic=False, norm_cfg=dict(type='BN'),
    ...
    train_cfg=dict(
        rpn=dict(
            _delete_=True,  # base config has rpn=None, must delete first
        ...
        # Switching from HungarianAssigner to MaxIoUAssigner with lower
        # IoU thresholds to improve recall on overlapping proposals.
        rcnn=[
            dict(
                assigner=dict(
                    type='MaxIoUAssigner',
    ...
    test_cfg=dict(
        rpn=dict(
            _delete_=True,  # base config has rpn=None, must delete first
            nms_pre=4000, max_per_img=3000,
        ),
        rcnn=dict(
            score_thr=0.001,
            nms=dict(type='soft_nms', iou_threshold=0.5, min_score=0.001),
            max_per_img=100,  # 300→100: num_proposals=100, k cannot exceed proposals*classes
        ),
    ),
)
