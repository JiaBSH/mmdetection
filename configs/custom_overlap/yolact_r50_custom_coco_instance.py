# YOLACT for dense overlapping instance segmentation.
#
# YOLACT uses ProtoNets + coefficient prediction for real-time instance
# segmentation. Its Fast NMS can be aggressive with overlapping instances,
# so we switch to standard NMS with higher thresholds and more detections.
#
# Overlap changes vs configs/custom/:
#   - test_cfg: score_thr=0.01 (from 0.05), max_per_img=300 (from 100)
#   - Use standard NMS with higher iou_threshold instead of Fast NMS
...
model = dict(
    ...
    test_cfg=dict(
        nms=dict(type='nms', iou_threshold=0.5),  # standard NMS
        score_thr=0.01,       # 0.05 → 0.01
        mask_thr=0.5,
...
train_pipeline = [
    ...
    dict(type='Expand', mean=img_norm_cfg['mean'], to_rgb=img_norm_cfg['to_rgb'],
         ratio_range=(1, 4)),
